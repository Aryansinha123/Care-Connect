...
export default function UserHistoryPage() {
  ...
  const [activeTab, setActiveTab] = useState('volunteers'); // 'volunteers' or 'donations'
  ...
  const fetchHistory = async (userObj) => {
    try {
      setLoading(true);
      const userId = userObj._id || userObj.id;

      // Fetch volunteer history
      const volunteerRes = await fetch(`/api/volunteer-participation?userId=${userId}`);
      const volunteerData = await volunteerRes.json();
      if (volunteerData.success) {
        setVolunteerHistory(volunteerData.participations || []);
      }

      // Fetch donation history
      const token = localStorage.getItem("token");
      const donationRes = await fetch(`/api/user/donations?email=${encodeURIComponent(userObj.email)}`, {
        headers: {
      ...
      if (donationData.success) {
        setDonationHistory(donationData.donations || []);
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-100">
      <div className="container mx-auto px-4 py-12 max-w-6xl">
        ...
        {/* Tabs */}
        <div className="flex gap-4 mb-6 border-b border-purple-200 justify-center">
          <button
          ...
          <div className="bg-white/80 backdrop-blur-sm p-8 rounded-3xl shadow-xl border border-white/20">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center">
                ...
                <h2 className="text-2xl font-bold text-gray-800">Volunteer History</h2>
              </div>
              <span className="bg-purple-100 text-purple-700 px-4 py-2 rounded-full text-sm font-medium">
                {volunteerHistory.length} {volunteerHistory.length === 1 ? 'Participation' : 'Participations'}
              </span>
            </div>

            {volunteerHistory.length > 0 ? (
              <div className="space-y-4">
                {volunteerHistory.map((participation) => {
                      ...
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex-1">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs text-gray-600">
                            ...
                            <div className="flex items-center">
                              <span className="mr-1">📅</span>
                              {request?.dateTime ? new Date(request.dateTime).toLocaleDateString() : 'Date not specified'}
                            </div>
                            <div className="flex items-center">
                              <span className="mr-1">🕐</span>
                              {request?.dateTime ? new Date(request.dateTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Time not specified'}
                            </div>
                          </div>
                        </div>
